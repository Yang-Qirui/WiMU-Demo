...
def parse_train_json_data(file_path):
    """解析训练JSON文件并提取坐标和值"""
...
def create_scatter_heatmap(heatmap_data, output_file='train_scatter_heatmap.png'):
    """创建散点图形式的热力图"""
    if not heatmap_data:
        print("没有数据可以绘制")
        return
    ...
    # 设置标题和标签
    plt.title('Training Data: Scatter Heatmap of Average Values by Coordinates', fontsize=16)
    ...
    print(f"训练数据散点热力图已保存为: {output_file}")
    
    # 显示图片
    plt.show()

def create_enhanced_scatter_heatmap(heatmap_data, output_file='train_enhanced_scatter_heatmap.png'):
    """创建增强版散点热力图，包含更多信息"""
    if not heatmap_data:
        print("没有数据可以绘制")
        return
    ...
    ax1.set_title('Training Data: Scatter Heatmap of Average Values', fontsize=14)
    ...
    ax2.set_title('Distribution of Average Values (Training Data)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"训练数据增强版散点热力图已保存为: {output_file}")
    plt.show()

def main():
    # 解析训练数据
    file_path = 'output/fine_tuned_train_result.json'
    heatmap_data = parse_train_json_data(file_path)
